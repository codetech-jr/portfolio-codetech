...
const PackageCard = ({ pkg }) => (
    <div
      className={cn(
        "relative flex flex-col h-full p-8 rounded-2xl border-2 transition-all duration-300 bg-[#1B1F3B] border-[#003B8D]",
        pkg.highlight && "border-[#00C6FF] shadow-2xl shadow-[#00c6ff]/10 scale-105 z-10"
      )}
    >
        {/* ... contenido del componente sin cambios ... */}
        {pkg.highlight && (
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 text-sm font-semibold text-[#0C0C2C] bg-[#00C6FF] rounded-full">
                Recomendado
            </div>
        )}
        <div className="flex flex-col flex-grow text-center">
            <h3 className={cn("text-2xl font-bold mb-2", pkg.highlight ? 'text-[#00C6FF]' : 'text-[#FFFFFF]')}>
                {pkg.name}
            </h3>
            <p className="text-[#A3A8CC] mb-6 text-sm">{pkg.description}</p>
            <ul className="space-y-3 text-left mb-8 text-[#A3A8CC] text-sm">
                {pkg.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                        <CheckIcon className="text-[#00C6FF] mt-1 flex-shrink-0" />
                        <span>{feature}</span>
                    </li>
                ))}
            </ul>
        ...
        <div className="mt-auto text-center">
            <div className="mb-6 text-2xl font-extrabold text-[#FFFFFF]">
                <span className="text-[#00C6FF]">{pkg.price}</span>
            </div>
            <Button
                size="lg"
                className={cn(
                    "w-full font-bold",
                    pkg.highlight
                    ? 'bg-[#00C6FF] text-[#0C0C2C] hover:bg-[#00C6FF]/90'
                    : 'bg-transparent border-2 border-[#003B8D] text-[#A3A8CC] hover:bg-[#003B8D] hover:text-white'
                )}
            >
...
const AddonCard = ({ addon }) => (
    <div className="p-6 rounded-xl bg-[#1B1F3B] border border-[#003B8D] flex flex-col">
        <h4 className="text-lg font-semibold text-[#00C6FF] mb-2">{addon.title}</h4>
        <p className="text-[#A3A8CC] text-sm flex-grow mb-4">{addon.description}</p>
        <div className="mt-auto font-bold text-right text-white">{addon.price}</div>
    </div>
);
    ...
    <section id="servicios" className="py-24 bg-[#0C0C2C]">
      <div className="container px-4 mx-auto">
        ...
        >
          ...
          <p className="max-w-2xl mx-auto text-[#A3A8CC] mb-12">
            Soluciones a la medida para llevar tu proyecto al siguiente nivel. Elige el paquete que mejor se adapte a tus necesidades.
          </p>
        </motion.div>
        
        {/* 4. Animar el grid de paquetes con 'staggering' */}
        <motion.div
          className="grid items-stretch grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3"
        ...
        <div className="mt-24">
          {/* 5. Animar el encabezado de los add-ons */}
          <motion.div
            className="text-center"
            initial="hidden"
            whileInView="show"
          ...
          {/* 6. Animar el grid de add-ons con 'staggering' */}
          <motion.div
          ...
          >
            {addons.map((addon) => (
              <motion.div key={addon.title} variants={itemVariants}>
